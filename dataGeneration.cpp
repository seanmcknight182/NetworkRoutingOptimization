...
// points for putting into an array, could be done with a multidimensional array but that would be such a pain
struct pointStruc{		
	int xValue = -1;
...
private:
	...
	int writable[105][6];		// writable[i][0] is the original y value
					// writable[i][1] is how many points have that y value
					// writable[i][2-5] is the index in theArray[] that have this y value
...
public: 
	...
	void generatePoints(){
		...
		for(int i = 1; i < 299; i++){	// theArray[0] and [299] are preset to always be 0,0 and 99,99 respectively
			...
			writable[writableIndex][1] = writable[writableIndex][1] + 1;	// updates number of points with this y value
			...
			if(writable[writableIndex][1]>3){
				...
				placeholder = writable[acceptableWritePos][5];	
				writable[acceptableWritePos][5] = writable[writableIndex][5];
				writable[writableIndex][5] = placeholder;
				acceptableWritePos = acceptableWritePos - 1;
				i3++;
